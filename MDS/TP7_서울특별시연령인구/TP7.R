setwd("C:\\Users\\yoonjun\\Desktop\\jun\\4_1\\machine_learning\\논문\\TP7_서울특별시연령인구")
...
# 연령별 인구 수 MinMax 정규화.
data.2015[3:23] <- normalize(data.2015[3:23])
data.2016[3:23] <- normalize(data.2016[3:23])
data.2017[3:23] <- normalize(data.2017[3:23])
data.2018[3:23] <- normalize(data.2018[3:23])
data.2019[3:23] <- normalize(data.2019[3:23])
...
make_plot <- function(mds,n) {
  plot(mds, type='n', ann=F, axes=F) +
    title(main=paste(n, "년 지역별 유사도 연령별 인구", sep=""), cex.main=2, family = "NanumGothic") +
...
p <- ggplot(data=mds.all, mapping = aes(x=x, y=y)) + 
  theme_void() +
  ggtitle("2015~2019년 지역별 유사도 연령별 인구") +
...
animate(p, end_pause = 20, height = 800, width = 800)
